...
1.  **Use RStudio Projects:**
    ...
    -   To create new RStudio project: "*File* \> *New Project...* \> *New Directory* \> *New Project* \> (*provide a name and location*) \> *create project"*.
...
3.  **Use a consistent folder structure within each RStudio project:** 
    ...
    -   Project/Analysis name
        ...
        -   data -- any data sets used in the project or analysis 
            ...
            -   raw data -- raw data is treated as a read only file (never adjust the contents of this file). If changes are required, they should be done via code and communicated back to the database manager for correction 
...
6.  **Have a naming convention for local variables, global variables, constants, and functions:**
    ...
    -   Avoid numbers in variable names unless they are informative.
        ...
        -   Incremental names are not informative (e.g., what are object names "S1", "S2", and "S3"). If used, their meaning should be described in a comment.
...
10. **Format dates using the international date/time standard format (ISO 8601), which is yyyy-mm-dd:** Dates regularly cause errors. Make sure to clean dates at the beginning of a script and make all formats consistent since the dates you receive will likely not be in the same format.

11. **Use Quarto:** When appropriate, use Quarto to generate output documents in Word, PDF, and html formats. Using Quarto allows you to include code and analyses within the file that produces the output document, reducing the likelihood of copy and paste errors. Using Quarto also increases efficiency by allowing you to easily regenerate the document following a data set update or tweak to the code.
...
-   RStudio snippets can be used to insert a standard header and template in a new script. To create a template navigate to "*Tools* \> *Edit Code Snippets...* \> (*paste the following at the bottom of the pop-up*) \> *Save*".
    ...
    To use the snippet type `rscript` slowly in a blank R script and hit "Enter" when the command auto-completes. A standard header/code template will be added to your script with the cursor placed where the code description should be entered (see @fig-snippet_output). After you have entered an appropriate description pressing the "Tab" key will take the cursor to the area where the analyst name should be entered, a third "Tab" keystroke will take the cursor to the area where packages can be specified and a forth "Tab" keystroke will take the cursor to the section where you can begin writing your code.

![The output of the rscript snippet on a blank R script.](snippet_output.png){#fig-snippet_output}

-   Add comments for obscure packages explaining the functionality it provides. You can also specify package names when using functions to make the dependency clear and/or avoid conflicts between packages (e.g. `dplyr::rename` vs. `rename`).
...
-   Place spaces around all binary operators (`=`, `+`, `-`, `<-`, etc.). Do not place a space before a comma, but always place one after a comma. Place a space before left parenthesis or curly bracket, except in a function call. There should be a hard return after pipe operators.

    ```{r}
    # An example of R script spacing
    human_age <- function(dog_age){
      #args: The age of a medium sized dog
      #return: The equivalent human age for the dog
